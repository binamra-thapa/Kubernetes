# Redis Operator with Operator SDK: Step-by-Step with Go

This guide documents how to build a Redis operator from scratch using the Operator SDK in Go. It includes command explanations, folder structure, example content, and expected outputs.
## 🧰 Prerequisites
...
operator-sdk init --domain=yourorg.io --repo=github.com/yourorg/redis-operator
```
...
Creates:
```
config/crd/bases/redis.yourorg.io_customclusters.yaml
...
```yaml
# config/samples/redis_v1alpha1_customcluster.yaml
apiVersion: redis.yourorg.io/v1alpha1
kind: customcluster
metadata:
  name: redis-example
spec:
...
```bash
kubectl apply -f config/samples/redis_v1alpha1_customcluster.yaml
```

Now your operator will react and (eventually) create Redis resources in `Reconcile()`.
...
│   ├── crd/
│   │   ├── bases/
│   │   │   └── redis.yourorg.io_customclusters.yaml  # ← Your CRD YAML (generated by make manifests)
│   │   └── kustomization.yaml
│   │
│   ├── default/
│   │   └── kustomization.yaml
...
│   │
│   ├── samples/
│   │   ├── redis_v1alpha1_customcluster.yaml  # Example custom resource
│   │   └── kustomization.yaml
│   │
│   └── scorecard/
│       └── patches/
